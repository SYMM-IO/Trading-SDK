...
/**
 * Solvers-page card: read just `openInterest` and `totalCap` for one market.
 * Thin selector over `/notional_cap/{symbolId}` — same call as
 * {@link ReadNotionalCap}, smaller surface.
 */
export function ReadOpenInterest() {
  ...
  const query = useOpenInterestBySymbolId({
    symbolId,
  ...
  return (
    <MethodCard
      testId="method-getOpenInterest"
      name="getOpenInterestBySymbolId"
      mutability="view"
      description="Read just the open interest and total cap for one market."
      wide
    >
      <div className="grid gap-4 sm:grid-cols-[minmax(0,1fr)_auto]">
        <Field label="market" htmlFor="input-open-interest-market">
          <MarketSelect
            idPrefix="input-open-interest-market"
            value={marketId}
            items={marketItems}
        ...
        <div className="flex items-end gap-2">
          <Button
            ...
            data-testid="button-open-interest-toggle-polling"
          >
      ...
      <div className="flex flex-wrap items-center gap-2">
        <Button
          ...
          data-testid="button-read-open-interest"
        >
          {query.isFetching ? (
            <>
      ...
      <ResultPanel testId="result-getOpenInterest" query={query} />
    </MethodCard>
  );
}

function ResultPanel({ testId, query }: { testId: string; query: ReturnType<typeof useOpenInterestBySymbolId> }) {
  if (query.isLoading) {
    return (
  ...
  if (!query.data) {
    return <ResultNote testId={`${testId}-idle`}>Select a market to read open interest.</ResultNote>;
  }
