...
Stack: Next.js + React + Tailwind. Built against `@symm-frontier/react` and `@symm-frontier/ui`.

## Rules

- **Never import `@symm-frontier/core` directly.** This is Hard Rule 6 in the root `AGENTS.md`. If you need a `core` API, expose it through `@symm-frontier/react` first (as a hook or provider), then use it here. The package.json `dependencies` for this app must not list `@symm-frontier/core`.
- **No SDK-shaped logic in app code.** If a piece of logic could be reused by another consumer (another app, a third-party integrator, a CLI), it belongs in `@symm-frontier/core` or `@symm-frontier/react`. Push it down before shipping it here.
...
- **Design system usage.** Reusable visual primitives come from `@symm-frontier/ui`. Do not re-implement a Button locally; if a primitive is missing, propose it in `@symm-frontier/ui`.
