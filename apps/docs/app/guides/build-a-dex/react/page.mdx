...
<Callout type="info">
  **What the affiliate address is — and why it's how you get paid.** In SYMM perps, trades are opened through frontends —
...
It's mandatory by design: **the SDK throws (`AFFILIATE_ADDRESS_REQUIRED`) if a supported chain's affiliate is the zero
address or missing**, so a trade can never silently fall back to the protocol's default affiliate and lose your
attribution. Running your own frontend and earning a cut of trading fees means **registering your affiliate address**
with the protocol first, then setting it under `symmioConfig[chainId].addresses.affiliatesAddress`.
...
- [ ] Affiliate set under `SymmioProvider`'s required **`symmioConfig`** — each chain's `addresses.affiliatesAddress` a **registered, non-zero** address (the SDK throws on zero/missing), and a `getWalletClient` resolver wired for session-key signing.
