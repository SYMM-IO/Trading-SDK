...
- [ ] Affiliate set under `SymmioProvider`'s required **`symmioConfig`** — each chain's `addresses.affiliatesAddress` a **registered** address to earn fees (the SDK throws only on **missing**; zero is a no-affiliate sentinel with no fee share, a non-zero unregistered affiliate reverts on-chain at trade time), and a `getWalletClient` resolver wired for session-key signing.
  > > > > > > > main
