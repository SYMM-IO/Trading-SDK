...
<Callout type="info">
  **This guide walks the lowcap build (enigma / HyperEVM).** The same hooks build the **majors** DEX (rasa / Base) — see
...
Every hook here takes an optional `solverId` and follows the connected chain's **default solver** when you omit it — `enigma` on HyperEVM, `rasa` on Base. So the same app serves both products; a handful of things differ on **majors (rasa / Base)**:
...
- **Rasa-only reads.** Solver readiness, whitelist, uPnL, open interest, and price range live on the [Rasa Solver hooks](/react/rasa-solver) page and throw `UNSUPPORTED_BY_SOLVER` off a non-rasa solver.
...
Switch to target chain   ← wallet.chainId === targetChainId (Base majors / HyperEVM lowcap)
...
- **[`useWalletAccount`](/react/wallet#usewalletaccount)** → `{ address, chainId, isConnected, isReconnecting, isOnExpectedChain }`. `isOnExpectedChain` is true when the wallet is on **any** SDK-supported chain (HyperEVM **or** Base).
...
<Callout type="warning">
  **Serving both products? Gate on the _target_ chain, not just "any supported chain."** `isOnExpectedChain` and
  `useSwitchToSymmioChain` are chain-agnostic — a wallet on HyperEVM passes `isOnExpectedChain` even when you're building
  the **majors** DEX (Base). Pick the target chain from the product/solver you're showing, compare the wallet against it,
  and prompt a switch to that **exact** id (wagmi's `useSwitchChain`), so a majors user lands on Base, not HyperEVM:

```tsx
import { SymmioSupportedChainId } from "@symmio/trading-core";
import { useSwitchChain } from "wagmi";

// targetChainId = the chain of the product you're rendering (majors → Base, lowcap → HyperEVM).
...
if (isConnected && chainId !== targetChainId) {
  return (
    <button type="button" onClick={() => switchChain({ chainId: targetChainId })}>
      Switch to {isMajors ? "Base" : "HyperEVM"}
    </button>
  );
}
```
