...
It's mandatory by design: **the SDK throws (`AFFILIATE_ADDRESS_REQUIRED`) if a supported chain's affiliate is
missing** (present-but-zero is allowed), so a trade can never silently fall back to the protocol's default affiliate and
lose your attribution. The **zero address** is a testing placeholder — on-chain it is a **no-affiliate sentinel**, so the
trade still opens but you earn **no fee share**; what reverts is a **non-zero _unregistered_** affiliate, with
`PartyAFacet: Invalid affiliate` at trade time. Running your own frontend and earning a cut of trading fees means
**registering your affiliate address** with the protocol first, then setting it under
