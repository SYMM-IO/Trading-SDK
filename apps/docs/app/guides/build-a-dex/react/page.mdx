...
<Callout type="info">
  **This guide walks the lowcap build (enigma / Arbitrum).** The same hooks build the **majors** DEX (rasa / Base) — see
  [Solver & chain](#solver--chain-lowcap-vs-majors) below for what differs, and [Solvers &
  Chains](/core/concepts/solvers-and-chains) for the model.
</Callout>

## Solver & chain: lowcap vs majors

Every hook here takes an optional `solverId` and follows the connected chain's **default solver** when you omit it — `enigma` on Arbitrum, `rasa` on Base. So the same app serves both products; a handful of things differ on **majors (rasa / Base)**:

- **No Virtual Accounts.** A rasa sub-account is cross-margin (`SubAccountIsolationType.CUSTOM`). Skip `usePredictedNextVirtualAccount` and the TP/SL-against-VA step; read liquidation at the account level. Branch the UI on `subAccount.isolationType === SubAccountIsolationType.CUSTOM`, **not** on a chain id.
- **Prices come from Binance.** The mark-price hooks (`usePriceByName`, `useMarkPrices`) are provider-agnostic and serve Binance USD-M Futures for majors automatically — no code change. Narrow a tick on `provider` for Binance-only fields (`indexPrice`).
- **No TP/SL yet.** Base has no COH wallet, so conditional orders (TP/SL) are unsupported there today. (Notification history search **does** work — the unified `useSearchNotifications` hits the rasa solver's position-state endpoint by `solverId`; only the notification-_service_ `searchUrl` is enigma-only.)
- **Thousands of markets.** Majors list far more markets than lowcap — **virtualize** the market selector list so opening it stays instant.
- **Rasa-only reads.** Solver readiness, uPnL, open interest, and price range live on the [Rasa Solver hooks](/react/rasa-solver) page and throw `UNSUPPORTED_BY_SOLVER` off a non-rasa solver.

Everything else in this guide — the gate ladder, session keys, `useManagedQuotes`, the close flow — is identical across both.
...
- **The header opens a searchable selector, not a `<select>`.** Back it with `useMarkets().data`, label each row with the market's **`symbol`** field (the human ticker you display), and let the user filter on it.
- **Trade-form field order is fixed:** Side → Margin → Leverage → TP/SL → Slippage → Submit.

<Callout type="info">
  **The chart is its own guide.** See [Lowcap Chart](/guides/charts/lowcap). **Option A** — a DexScreener `<iframe>`
...
- **`SymmioProvider`** — props are **`symmioConfig`** (required), **`defaultChainId`**, **`getWalletClient`** (plus `children`). `symmioConfig` is the per-chain SYMMIO config, keyed by chain id; each supported chain must have `addresses.affiliatesAddress` **present** (the provider throws only when it is missing — `zeroAddress` works out of the box; a **registered** affiliate earns fees):
  ...
  const symmioConfig = {
    [SymmioSupportedChainId.ARBITRUM]: {
      // `zeroAddress` works out of the box — trades open, you earn no fee share.
      // Swap in your registered affiliate to collect fees (see the callout below).
...
<Callout type="info">
  **What the affiliate address is — and why it's how you get paid.** In SYMMIO perps, trades are opened through frontends —
  dApps, brokers, bots. The affiliate address is that frontend's **identity onchain**. It rides on every quote (the
  `affiliate` argument of `sendQuote`), so the protocol knows **who sourced the trade** and
...
<AffiliateLink>**register your affiliate address**</AffiliateLink> and swap it in under
...
Switch to target chain   ← wallet.chainId === targetChainId (Base majors / Arbitrum lowcap)
...
- **[`useWalletAccount`](/react/wallet#usewalletaccount)** → `{ address, chainId, isConnected, isReconnecting, isOnExpectedChain }`. `isOnExpectedChain` is true when the wallet is on **any** SDK-supported chain (Arbitrum **or** Base).
...
- **[`useSwitchToSymmioChain`](/react/wallet#useswitchtosymmiochain)** → `{ switchChain, status, error }`. Switches to the config's **default** chain — enough for a single-chain app.

A single `WalletButton` renders three states off `useWalletAccount()`: **not-connected**, **wrong-chain**, **connected**. When the wallet is on the wrong chain, render a **"Switch to _\<chain>_"** button that calls the switch action; it flips back to **connected** the moment the wallet reports the new chain.

<Callout type="warning">
  **Serving both products? Gate on the _target_ chain, not just "any supported chain."** `isOnExpectedChain` and
  `useSwitchToSymmioChain` are chain-agnostic — a wallet on Arbitrum passes `isOnExpectedChain` even when you're building
  the **majors** DEX (Base). Pick the target chain from the product/solver you're showing, compare the wallet against it,
  and prompt a switch to that **exact** id (wagmi's `useSwitchChain`), so a majors user lands on Base, not Arbitrum:

```tsx
import { SymmioSupportedChainId } from "@symmio/trading-core";
import { useSwitchChain } from "wagmi";

// targetChainId = the chain of the product you're rendering (majors → Base, lowcap → Arbitrum).
...
if (isConnected && chainId !== targetChainId) {
  return (
    <button type="button" onClick={() => switchChain({ chainId: targetChainId })}>
      Switch to {isMajors ? "Base" : "Arbitrum"}
    </button>
  );
}
```
...
The user-facing collateral flow for a lowcap DEX is just **deposit** and **withdraw** — funds live in the SubAccount's **available** balance, which instant trading spends directly. (Full picture of every balance and where it lives: [Balance Model](/core/concepts/balance-model).)
...
Prefer the Express-aware hooks when the selected chain config includes an Express service. Prepare and display the route,
then submit the exact route the user approved:

```tsx
import { useExpressWithdrawStatus, useWithdrawRoute, useWithdrawWithExpress } from "@symmio/trading-react";
...
function submit() {
  ...
  withdraw.mutate({ amount, receiver: walletAddress, preparedRoute: route.data });
}

const status = useExpressWithdrawStatus({
  user: subAccount,
  requestId: withdraw.data?.requestId ?? 0n,
  query: { enabled: withdraw.data?.route.kind === "express" },
});
```

The default route is classic immediate only when the account holds enough available balance and its cooldown is already
satisfied; otherwise it prefers `SAME_TX`, then `STANDARD`, then classic cooldown fallback. `WINDOWED` must be explicitly
added to `policy.optionPriority`. Express v1 is same-chain only. For `STANDARD`, keep the UI pending through Core
`FINALIZED`; only provider `PROCESSED` proves receiver payment. The hook implements that polling rule.

See [Withdraw hooks](/react/withdraw) for a complete component, configuration, cache invalidation, and failure behavior.
Use the restricted [browser proxy implementation](/core/express-withdraw#configuration) and configure its local root as
the chain's `expressWithdraw.url`; never expose a generic upstream pass-through.

The classic low-level flow remains available and is what the router selects when Express is unavailable:
...
<Callout type="warning">
  **Persist the session key — never keep it in memory only.** Back your `SessionKeyManager` with a **durable**
  ...
  Encrypt the stored private key — see [Security &amp; storage](/session-key/security).
</Callout>
...
| Constant / helper                                                     | Grants                                                                    |
| --------------------------------------------------------------------- | ------------------------------------------------------------------------- |
| `SEND_QUOTE_SELECTOR` / `SEND_QUOTE_WITH_AFFILIATE_AND_DATA_SELECTOR` | open a position (perps-core v0.8.6 / v0.8.5 chains respectively)          |
| `REQUEST_TO_CLOSE_POSITION_SELECTOR`                                  | close a position                                                          |
| `ADD_MARGIN_TO_NEXT_VA_SELECTOR`                                      | top-up margin                                                             |
| `useInstantTradeRequiredSelectors()`                                  | all three for the connected chain, in the order `grantDelegation` expects |

The open-leg selector is **per-chain** (the chain's `contractsVersion` decides which quote-send call its diamond and solver accept), so resolve the set with the hook instead of hardcoding `INSTANT_TRADE_REQUIRED_SELECTORS` / `LEGACY_INSTANT_TRADE_REQUIRED_SELECTORS` in a multi-chain flow.
...
- **[`useGrantDelegation()`](/react/instant-layer#usegrantdelegation)** — mutation. Variables: `{ account: { addr: subAccount, isPartyB: false }, delegatedSigner: sessionKeyAddress, selectors: useInstantTradeRequiredSelectors(), expiryTimestamp }`. `expiryTimestamp` is Unix seconds (a 24h–30d TTL keeps returning users from re-granting). On success the three `useIsDelegationActive` queries auto-invalidate and the gate flips to `true`.
...
function useTradingDelegation(subAccount?: Address) {
  ...
  // Chain-resolved: the open leg is sendQuote on v0.8.6, the legacy call on v0.8.5.
...
- **Session key** → `useInstantTradeRequiredSelectors()` (open, close, add margin — chain-resolved).
...
<Callout type="warning">
  **Before you show a Close button, confirm 1-click trading is enabled.** Closing is session-key-signed, so it lives
  behind the same delegation gate as the open form — and a position that survived from a prior session can still show up
  here with a stale key. When the trading delegation is **inactive**, don't render a live Close action: swap it for an
  **"Enable trading"** button that prompts the user to grant the session-key delegation, then flips back to Close the
  moment the grant lands (see [§4 · Gate every write](#gate-every-write-not-just-the-open-form)). Skip this and the
  hedger accepts the close while the on-chain anchor reverts — a silent failure.
</Callout>
...
A per-position details view (a modal or side panel) is pure composition — no new fetch beyond the per-quote hooks below plus the `UnifiedQuote` fields:
...
| Pending funding      | **[`useQuotePendingFunding`](/react/symmio-contract#usequotependingfunding)** `({ quote })` → `data?.pendingNetReceived` (wei), income-positive: `> 0n` the position will receive it. Accumulated funding accrued since the last settlement; `null` for a quote that is not an active position. Settled funding is the separate [`useQuoteFunding`](/react/subgraph#usequotefunding) — show both, never their sum.                                                                                                                                                                                                                                                                                                                                                                                                 |

The amount / price values — the `UnifiedQuote` quantities and prices, `liquidationPrice`, the fees, and `pendingNetReceived` — are 18-decimal **wei** `bigint`; divide by `1e18` (`formatUnits`) to display. The exceptions are `useQuoteUpnlAndPnl`'s outputs — **`markPrice`, `upnl`, and `upnlPercent` are already decimal strings**, so pass them straight to your formatter without dividing.
...
- [ ] Affiliate set under `SymmioProvider`'s required **`symmioConfig`** — each chain's `addresses.affiliatesAddress` present (the SDK throws only on a **missing** field; `zeroAddress` is the correct no-affiliate default — trades open, no fee share), upgraded to a **registered** address when you monetize (registration earns a share of the trading fees), plus a `getWalletClient` resolver wired for session-key signing.
...
- [ ] Delegation checked for all three selectors; granted at once via `useInstantTradeRequiredSelectors()`; expiry surfaced via `useDelegationExpiry`.
...
| Symptom                                                                            | Cause                                                            | Fix                                                                                              |
| ---------------------------------------------------------------------------------- | ---------------------------------------------------------------- | ------------------------------------------------------------------------------------------------ |
| `Cannot read properties of undefined (reading 'ARBITRUM')`                         | `SymmioSupportedChainId` imported from react                     | Import from `@symmio/trading-core`                                                               |
...
| Users told to register an affiliate before they can trade                          | Affiliate treated as a prerequisite                              | `zeroAddress` is the valid no-affiliate default; register to earn fees ([Providers](#providers)) |
| Trade form shows zero margin right after a deposit                                 | Funds allocated via `useAllocate` / `useDepositAndAllocate`      | Instant trading spends the **available** balance — deposit only ([§3](#3-collateral))            |
| Balance panel driven by `allocatedBalance`                                         | Classic-pool read used for the instant flow                      | Show `useAccountBalanceOf` (available); see the [Balance Model](/core/concepts/balance-model)    |
