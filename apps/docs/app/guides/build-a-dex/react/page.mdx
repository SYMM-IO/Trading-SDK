...
`Max` is not the raw balance — the request must survive the open fee and, for SHORT, a worst-case slippage fill. (The close fee is not shaved — the contract charges it at close from the position, not at open.) The SDK does the shave for you with [`useAvailableInstantOpenMargin`](/react/margin#useavailableinstantopenmargin):
...
It applies `available = balance × max(0, 1 − slippageFactor) × max(0, 1 − leverage × openFee)` — `slippageFactor = slippage` on SHORT and `0` on LONG (the open fee is charged on the leveraged notional; SHORT also caps for the worst-case fill; the close fee is charged at close, not shaved here). Because it reads the balance with `live: true`, the `Max` **refetches automatically when the open/close settles on-chain** — no manual refresh.
...
To show what the close will cost, preview the close-fee legs with **[`useInstantCloseFees`](/react/instant-layer#useinstantclosefees)** — `({ subAccountAddress: partyA, market: { id }, quantity, openedAt: quote.createTimestamp })` prices the platform close fee plus, on lowcap, the holding-time solver fee and the flat static leg, all from cached reads.
