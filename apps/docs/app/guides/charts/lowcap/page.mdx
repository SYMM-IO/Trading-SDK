...
Lowcap markets are not listed on a reference exchange, so there are no venue candles to fetch — the SDK's [Binance `CandleSource`](../majors) has nothing to serve for them. The price has to come from the liquidity pool the token actually trades in, and you pick the rendering approach that fits your UI budget and DX preferences.

<Callout type="info">
  Charting a **major**? Use the [Majors Chart](../majors) guide instead — the SDK fetches and streams real OHLCV for
  those. `source.getSymbol(marketName)` returning `undefined` is how a source tells you a market belongs on this page
...
export function LowcapOhlcChart(props: Props) {
  ...
  useEffect(() => {
    ...
    // Lightweight Charts v5. On v4 this is `chart.addCandlestickSeries()`.
...
- [Majors Chart](../majors) — the other half: markets that _are_ listed on a reference exchange, where the SDK fetches and streams the bars for you.
