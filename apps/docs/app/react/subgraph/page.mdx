...
Funding totals — paid, received, net — for one quote across its lifetime. Disabled while `quoteId` is `undefined` (an off-chain row).

```ts
const { data, isLoading } = useQuoteFunding({ quoteId: 42n });
/** `data` is `QuoteFundingData | null`: { quoteId, paid, received, net }, all wei. */
...
**Sign convention** — shared with `useQuoteFunding` and `useQuotesFunding` above: `net = paid − received`, so a **positive** `net` means the user net-**paid** funding. This matches `QuoteFundingData.net` and the on-chain `int256`. A UI that colors "money in" green must negate at the render layer — the SDK never flips the sign.
...
function GroupedPositionFunding({ group }: { group: QuoteGroup }) {
  ...
  /** `funding.net` is a LOWER BOUND until `isComplete` is true — never render it as final before checking. */
...
`funding` is a [`QuoteGroupFunding`](../../core/quotes/quote-group-funding): `paid` / `received` / `net` (wei), `resolvedCount`, `expectedCount`, `missingQuoteIds`, and `isComplete`. Treat `isComplete: false` as **"funding unknown"**, not "no funding" — an all-optimistic or empty group reports `false` with `net: 0n`. `rows` is the per-child `QuoteFundingData | null`, aligned 1:1 with `group.quotes`.
...
for (const row of data?.rows ?? []) {
  ...
  /** net > 0n → the user paid funding on this tick. `row.rate` is the signed rate applied (wei). */
}
```
