...
Two hooks. `useSupportsListingService` says whether the feature exists on the connected target; `useListingMarkets` reads a page of the catalog.
...
Value types and enums come from `@symmio/trading-core`: `ListingMarket`, `ListingMarketPage`, `ListingMarketStatus`, `ListingDepositChainId`, `ListingMarketSortField`, `ListingMarketFilters`, and `LISTING_VALUE_DECIMALS` — the fixed-point scale every money and rate field on a row uses.
