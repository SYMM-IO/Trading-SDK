...
Five hooks. `useSupportsListingService` says whether the feature exists on the connected target; `useListingMarkets` reads a page of the public catalog; `useAuthenticateListing` signs the user in (SIWE) and returns their access token; `useUserListingMarkets` reads **Your Pools** — the authed, per-wallet view of the catalog — using that token; `useAddMarket` creates a pool by listing a new token, also with that token.

Three of them are reads. `useAuthenticateListing` and `useAddMarket` are **mutations** — the first prompts a wallet signature, the second submits an authed create-pool write.
...
Value types and enums come from `@symmio/trading-core`: `ListingMarket`, `ListingMarketPage`, `UserListingMarket`, `UserListingMarketPage`, `CreatedPool`, `ListingMarketStatus`, `ListingDepositChainId`, `ListingMarketSortField`, `ListingMarketFilters`, and `LISTING_VALUE_DECIMALS` — the fixed-point scale every money and rate field on a row uses.
