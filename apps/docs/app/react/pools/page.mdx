...
Revenue is no longer one of them: the current solver generation serves revenue **per market only** (`/revenue/{symbolId}`), so [`useSolverRevenue`](../solvers#usesolverrevenue) belongs on the pool-detail row next to volume — keyed by the pool's `symbolId` — not in the headline strip.
...
- **A revenue figure is one market's, never the protocol's.** `useSolverRevenue` requires a `symbolId` and answers for that market alone — if you sum several markets, label the result with the markets it covers rather than calling it protocol revenue.
