...
```ts
import { normalizeSymmError } from "@theoldvarorg/react";
...
`@theoldvarorg/react` re-exports `SymmError` from `core` for convenience. `normalizeSymmError(symmError).kind` is always `"sdk"`.

```ts
import { SymmError } from "@theoldvarorg/react"; // same as `from "@theoldvarorg/core"`
```
