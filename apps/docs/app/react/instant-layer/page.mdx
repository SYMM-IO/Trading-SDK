...
The result is a `kind`-discriminated union. Both kinds carry `platformOpenFee`, `platformCloseFee`, `notional`, and `totalFee`; an `"enigma"` (lowcap) result adds `openSolverFee`, `closeSolverFee`, and `expectedSettlementLoss`.
