...
React-query wrappers over `@symm-frontier/core`'s InstantLayer slice.
...
## `useDelegationExpiry(params)`

Read the raw expiry timestamp for one account, delegate, and selector.

```tsx
import { useDelegationExpiry } from "@symm-frontier/react";
...
## `useIsDelegationActive(params)`

Read whether the contract considers a delegation active.

```tsx
import { useIsDelegationActive } from "@symm-frontier/react";
...
## `useGrantDelegation(options?)`

Submit an InstantLayer `grantDelegation` transaction.

```tsx
import { useGrantDelegation } from "@symm-frontier/react";
