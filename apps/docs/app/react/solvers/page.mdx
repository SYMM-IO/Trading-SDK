...
The resolved `SolverCapabilities` object (`{ groupClose, limitOrder, listingService }`, all definite booleans).

```tsx
const { limitOrder, groupClose, listingService } = useSolverCapabilities();
```
...
Boolean shorthand — whether this solver's markets are the lowcap **Pools**, i.e. whether it uses the chain's listing backend. Gate the Pools tab on it so the feature hides where it is unavailable instead of erroring at request time.

```tsx
const hasPools = useSupportsListingService();
```

It checks only the solver's declaration. Core's `supportsListingService` additionally requires the chain to carry a `listing` block. Every request [`useListingMarkets`](/react/pools) issues runs that stricter check through `resolveListingService` — the throwing form — which is why a read against a target without Pools surfaces as `LISTING_UNSUPPORTED` / `LISTING_NOT_CONFIGURED` rather than as a `false`. Today only enigma on HyperEVM has both halves. See [Pools listing service](/core/concepts/solvers-and-chains#pools-listing-service).
