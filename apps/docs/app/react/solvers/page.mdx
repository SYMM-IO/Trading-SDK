...
Each hook takes an optional `solverId` and defaults to the connected chain's solver (`enigma` on HyperEVM, `rasa` on Base). `useMarkets` returns the `Market` union — narrow on `kind` for solver-specific fields. Endpoints only the rasa solver serves (readiness, uPnL, open interest, price range) are on the [Rasa Solver hooks](/react/rasa-solver) page; see [Solvers & Chains](/core/concepts/solvers-and-chains) for the model.
