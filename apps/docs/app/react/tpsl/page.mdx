...
A step's lifecycle is `queued → submitting → confirming → done`. `set()` resolves at the **`confirming`** boundary — the handler has accepted every request — and confirmation continues asynchronously as the WebSocket reports the transition each step is waiting for (_live_ for a write, _gone_ for a cancel, so a shared socket cannot cross-confirm).

`confirming` outranks `partial`: a failure on one leg does not make the run terminal while another still awaits its report. `acceptedCount` excludes failures — read `failedCount` for those. `retryFailed()` re-runs only the failed legs, merged onto the existing steps so successful ones keep their state, and re-diffed against fresh children so a leg that confirmed via a race is dropped.
