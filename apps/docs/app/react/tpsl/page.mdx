...
React-query wrappers over `@symm-frontier/core`'s TP/SL slice, plus a shared Zustand store that keeps every render for one quote coherent — even across the temp → on-chain anchor transition.
...
```ts
import { useTpSlRecord } from "@symm-frontier/react";
...
```tsx
import { useSetQuoteTpSl } from "@symm-frontier/react";
...
```tsx
import { useDeleteQuoteTpSl } from "@symm-frontier/react";
...
```tsx
import { useInstantOpenWithTpSl } from "@symm-frontier/react";
...
```tsx
import { useWatchTpSlNotifications } from "@symm-frontier/react";
