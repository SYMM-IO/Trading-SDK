...
- **Required.** Every supported chain must set a non-zero `addresses.affiliatesAddress`, or the provider throws `AFFILIATE_ADDRESS_REQUIRED` (via `createConfig`). The affiliate is mandatory in the type — a chain entry without one is a compile error.
