...
- **Required.** Every supported chain must set `addresses.affiliatesAddress`, or the provider throws `AFFILIATE_ADDRESS_REQUIRED` (via `createConfig`) — the field must be present, not merely non-zero. The affiliate is mandatory in the type — a chain entry without one is a compile error.
