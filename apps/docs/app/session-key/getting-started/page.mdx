...
await grantDelegation(config, {
  ...
  delegatedSigner: state.publicAddress,
  selectors: getSessionKeySelectors(config, { chainId }),
  expiryTimestamp: BigInt(Math.floor(expiresAtMs / 1000)),
  gasless: true,
});
```

`gasless: true` relays the grant, so onboarding costs no native gas. Derive `expiryTimestamp` from the key's own
`expiresAt` — a delegation that outlives the key leaves stale authority on-chain, and one that expires first breaks the
key silently.

Because the contract enforces that `grantDelegation` is owner-only, this is the **only** wallet prompt the key ever
costs. Every write covered by those selectors is promptless afterwards.
