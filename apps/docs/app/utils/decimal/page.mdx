...
import {
  ...
  toDecimal,
  ZERO_DECIMAL,
} from "@symm-frontier/utils/decimal";
```

> **Naming overlap with viem.** `formatUnits` / `parseUnits` / `formatEther` / `parseEther` here **return / accept `Decimal`**, not strings. The root `@symm-frontier/utils` barrel intentionally does not re-export them so they never collide with viem's same-named functions. Always import from the `/decimal` subpath.
