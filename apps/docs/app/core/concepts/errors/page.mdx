...
class SymmApiError extends SymmError {
  readonly status: number; // HTTP status (0 when there was no response: network failure or timeout)
  readonly statusText: string;
  readonly responseData: unknown; // raw response body (solver / hedger error payload)
  readonly url: string;
  readonly method: string;
  readonly retryAfterMs: number | null; // Retry-After delay in ms, when the response carried one
}
```
...
- **`retryAfterMs`** is parsed from a `Retry-After` response header, in either delay-seconds or HTTP-date form. It is `null` when there is no usable header. A cross-origin browser reads the header only when the server lists it in `Access-Control-Expose-Headers`, so `null` means "unknown": still back off before retrying a `429` or `503`.
- **`cause` keeps the axios request config**, request headers and body included (a Pools action's `Authorization: Bearer` access token, for example). Redact it before forwarding the error to third-party logging.
- **Gasless HTTP failures are the exception:** their `cause` is a sanitized snapshot (`name`, `message`, `code`, `status`, `method`, `url`) with no request headers or body. Their `responseData` is still the raw response body, and it can echo request fields — see [Reading the vendor detail](/core/gasless/errors-and-fallback#reading-the-vendor-detail).
...
| `COLLATERAL_DECIMALS_UNSUPPORTED` | `collateralToCore18`, `core18ToCollateral`, `withdrawAuto`                       | Collateral decimals are not an integer from 0 to 18. |
...
| `FETCH_LISTING_MARKET_DETAIL_FAILED`                    | `getListingMarketDetail`                     | The pool-detail request failed.              |
| `FETCH_POOL_TRANSACTIONS_FAILED`                        | `getPoolTransactions`                        | The pool transaction-history request failed. |
