...
| `AMBIGUOUS_FUNDING`         | `prepareInstantOpenParams`, `getInstantOpenFees`                                 | Both `initialMargin` and `fund`, or both `availableBalance` and `fund`, were passed. |
| `INITIAL_MARGIN_REQUIRED`   | `prepareInstantOpenParams`, `getInstantOpenFees`                                 | Neither `initialMargin` nor `fund` was passed.                                       |
| `FULL_BALANCE_UNSUPPORTED`  | `prepareInstantOpenParams`, `getInstantOpenFees`                                 | `fund` or automatic-fallback `availableBalance` was used on a non-lowcap solver.     |
| `QUOTE_CONSTRAINT_VIOLATED` | Full-balance sizing in `prepareInstantOpenParams` / `getInstantOpenFees`         | The full-balance-sized quantity violates a published market constraint.              |
