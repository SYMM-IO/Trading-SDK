...
| `AMBIGUOUS_FUNDING`         | `prepareInstantOpenParams`, `getInstantOpenFees`                                 | Both `initialMargin` and `fund` were passed — provide exactly one.      |
| `INITIAL_MARGIN_REQUIRED`   | `prepareInstantOpenParams`, `getInstantOpenFees`                                 | Neither `initialMargin` nor `fund` was passed.                          |
| `FULL_BALANCE_UNSUPPORTED`  | `prepareInstantOpenParams`, `getInstantOpenFees`                                 | Full-balance `fund` was used on a non-lowcap (majors) solver.           |
