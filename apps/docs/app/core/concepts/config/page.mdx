...
Per-chain SYMMIO configuration, keyed by chain id, deep-merged onto the SDK's built-in defaults. Each entry may override that chain's `contractsVersion`, `addresses`, `subgraphs`, `solvers`, `priceService`, and `muon` endpoints — point at a staging subgraph, override an address, and so on. `contractsVersion` (`"0.8.5" | "0.8.6"`) declares the perps-core generation the chain's deployment runs — built-ins: Base is `"0.8.5"`, while Arbitrum is `"0.8.6"` — and drives which quote-send call the instant-open flow signs, the session-key selector set, and the withdraw-request decode; see [Solvers & Chains](/core/concepts/solvers-and-chains#contracts-version-is-a-chain-axis). **Notifications config is per-solver**, so it lives under `solvers[solverId].notifications` (resolve it at runtime with `config.getSolver({ chainId, solverId }).notifications`), not at the chain level.
