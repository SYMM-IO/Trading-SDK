...
- **Required.** Every supported chain must set `addresses.affiliatesAddress`, or `createConfig` throws `AFFILIATE_ADDRESS_REQUIRED` — the field must be present, not merely non-zero.
