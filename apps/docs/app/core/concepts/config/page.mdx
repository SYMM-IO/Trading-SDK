...
Per-chain SYMMIO configuration, keyed by chain id, deep-merged onto the SDK's built-in defaults. Each entry may override that chain's `addresses`, `subgraphs`, `solvers`, `priceService`, and `muon` endpoints — point at a staging subgraph, override an address, and so on. **Notifications config is per-solver**, so it lives under `solvers[solverId].notifications` (resolve it at runtime with `config.getSolver({ chainId, solverId }).notifications`), not at the chain level.
