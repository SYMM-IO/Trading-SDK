...
- **Required.** Every supported chain must set a non-zero `addresses.affiliatesAddress`, or `createConfig` throws `AFFILIATE_ADDRESS_REQUIRED`.
...
- `AFFILIATE_ADDRESS_REQUIRED` — a supported chain's `symmioConfig[chainId].addresses.affiliatesAddress` was missing or the zero address.
