...
- `AFFILIATE_ADDRESS_REQUIRED` — a supported chain's `symmioConfig[chainId].addresses.affiliatesAddress` was missing. (The zero address is accepted — a no-affiliate sentinel, no fee share; only a **non-zero unregistered** affiliate reverts on-chain at trade time.)
