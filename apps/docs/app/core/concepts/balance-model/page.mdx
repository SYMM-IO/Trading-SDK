...
- **Trade-form `Max`** — **not** the raw available balance. Use [`useAvailableInstantOpenMargin`](/react/margin#useavailableinstantopenmargin), which shaves the balance for the open fee and worst-case slippage (the close fee is charged at close from the position, not shaved here).
