...
When netting a funding row, the convention is `net = fundingPaid − fundingReceived`: a **positive** net means the user net-**paid** funding on that tick. This matches [`QuoteFundingData.net`](/core/subgraph/get-quote-funding) and the on-chain `int256`. A UI that colors "money in" green wants the inverse — do that inversion at the render layer, never here.
