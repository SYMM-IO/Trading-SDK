...
There is no `solverId`. The inventory service is configured per chain and no solver capability gates it, unlike the Pools listing backend where availability has two halves.
