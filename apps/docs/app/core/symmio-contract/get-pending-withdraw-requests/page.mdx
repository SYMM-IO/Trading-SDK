...
<Returns type="readonly WithdrawRequest[]">
  ...
  <Property name="advancedAmount" type="bigint" optional>
    Collateral already advanced to the provider before cooldown expiry (express credit-line flow). Added in perps-core
    v0.8.6 — **`undefined` on a `contractsVersion: "0.8.5"` chain**, whose contracts predate withdraw advances; the SDK
    decodes the legacy struct there and adds nothing to it.
  </Property>
</Returns>
