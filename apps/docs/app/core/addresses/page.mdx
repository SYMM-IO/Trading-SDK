...
| `affiliatesAddress`   | `Address` | Your frontend's affiliate identity (fee attribution). **Required** per chain via `createConfig`'s `symmioConfig[chainId].addresses.affiliatesAddress`. |
