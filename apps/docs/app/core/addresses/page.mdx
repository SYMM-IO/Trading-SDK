...
<Properties>
  ...
  <Property name="affiliatesAddress" type="Address">
    ...
    `symmioConfig[chainId].addresses.affiliatesAddress` — the field must be present (`createConfig` throws
    `AFFILIATE_ADDRESS_REQUIRED` when it is missing). The zero address is accepted as a no-affiliate test placeholder;
    <AffiliateLink>register your affiliate</AffiliateLink> to whitelist your frontend's domain and earn a share of the
    trading fees.
  </Property>
...
- `affiliatesAddress` — `0xBcB033C9154401fA000a1Ae60843f79f45741b7c` (built-in default; override it per chain via the required `symmioConfig[chainId].addresses.affiliatesAddress` — set your <AffiliateLink>registered affiliate</AffiliateLink> to whitelist your domain and earn a share of the trading fees)
