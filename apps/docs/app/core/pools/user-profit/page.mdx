...
<Returns type="Promise<UserPoolProfit>">
  ...
  <Property name="availableLpAmount" type="bigint">
    The LP shares free to withdraw right now — `userLpAmount − pendingWithdrawLpAmount`, floored at `0n`. **Derived by
    the SDK**, not a field the service returns; it is the ceiling a [`withdrawLp`](/core/pools/withdraw) request may
