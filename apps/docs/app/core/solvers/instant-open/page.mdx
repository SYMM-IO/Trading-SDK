...
On a `contractsVersion: "0.8.6"` chain the Enigma flow signs `sendQuote` carrying `solverFeeCaps` (`SolverFeeCaps` — 18-decimal rate caps the quote authorizes for the solver's open/close fees; the wizard fills the market minimums, and omitting it means zero caps, which a fee-charging solver rejects). On a `"0.8.5"` chain the flow signs the legacy `sendQuoteWithAffiliateAndData` and `solverFeeCaps` is ignored.
