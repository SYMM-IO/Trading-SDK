...
### `getUserSubAccounts(config, params)`
...
### `getAccountBalanceOf(config, params)`

Read raw `balanceOf(account)` from the SYMMIO core/diamond contract.

```ts
import { getAccountBalanceOf } from "@symm-frontier/core";
...
```ts
import { allocate } from "@symm-frontier/core";
...
```ts
import { deallocate, getDeallocateUpnlSig } from "@symm-frontier/core";
