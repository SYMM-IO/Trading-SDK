...
## And a companion: `@theoldvarorg/utils`
...
`@theoldvarorg/react` re-exports the most-used utils helpers (`formatTokenAmount`, `shortenAddress`, etc.) so React apps don't need to add a second dependency just to format a number.
