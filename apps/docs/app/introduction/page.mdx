...
SYMM Trading-SDK is an SDK-first workspace for [SYMMIO](https://symm.io). The underlying product is built by many vendors with different architectures and tools. Building UIs against that raw surface is complex, so this SDK hides the complexity behind a small, correct, reliable API.
