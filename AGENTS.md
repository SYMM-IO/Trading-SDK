...
- **Perps-core contract docs v0.8.6** — primary source of truth for **contract semantics, account/virtual-account behavior, events, lifecycle rules, and migration details**. Canonical docs live at https://github.com/SYMM-IO/perps-core/tree/version_0.8.6/docs/v0.8.6. Before implementing or reviewing contract/ABI behavior, read the relevant doc file(s) and cite them in the design proposal or explanation.
...
- **A React write hook forwards its mutation variables; it never accepts a field and drops it.** Build the core action's parameters with `{ ...variables, <hook-injected overrides> }` (injected values last, so they win) — never by hand-listing fields. If the hook takes a field over, remove it from the declared variables type with `Omit<>` instead. Hand-listing type-checks (an omitted optional field is a valid subset, and excess-property checking only fires on extras), so a dropped parameter fails silently: this is what killed the per-call `gasless` opt-in across 11 hooks while `execution.mode` kept working. Guarded by `packages/trading-react/src/transactions/write-hook-passthrough.test.tsx`.
- **Keep normalized solver/wire shapes flat — mirror the wire field names, no nested sub-objects.** A normalized read shape (`SolverSymbol`, `Market`, …) exposes every wire field as a top-level camelCase field named after the wire field (`hedger_fee_close_early_rate` → `hedgerFeeCloseEarlyRate`). Do **not** group related fields into a nested bag on the stored shape (e.g. a `closeFeeSchedule: {…}` object) — nesting hides fields from grep/discovery and misleads a consumer who knows the wire shape into hunting for a variable that "should" be top-level. When ≥2 of those fields are consumed together by a function, express the grouping as a **function param type** the flat shape structurally satisfies (e.g. `SolverCloseFeeRates`, so `getSolverCloseFeeRate(symbol, …)` type-checks) — the bundle lives in the signature, never in the stored data.
