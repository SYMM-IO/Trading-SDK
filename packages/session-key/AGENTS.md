...
`@theoldvarorg/session-key` is the framework-agnostic helper package for local EVM session keys. It owns key generation, import, runtime manager state, message/EIP-712 signing, and transfer payload helpers.
...
- **No SYMMIO contract logic.** Contract calls, address registries, and ABI fragments live in `@theoldvarorg/core`.
