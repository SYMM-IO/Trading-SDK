...
Has no React, no wagmi, no DOM — anything in this package can be imported from a Node script just as well as from a React app. Companion to [`@theoldvarorg/core`](../core) (which holds contract/ABI/address logic) and [`@theoldvarorg/react`](../react) (the React adapter).
...
```ts
import { formatTokenAmount, parseTokenAmount, rawToDecimal } from "@theoldvarorg/utils/amounts";
...
```ts
import { shortenAddress } from "@theoldvarorg/utils/address";
