...
1. **Define the normalized types — a shared base + one variant per kind, unioned.** Write a base interface of the fields **every** kind returns; each kind's type `extends` it and adds a `kind` discriminant plus only that kind's exclusive fields; union them:
   ...
   **Keep the shape flat.** Every wire field is a top-level camelCase field named after the wire field (`hedger_fee_close_early_rate` → `hedgerFeeCloseEarlyRate`); never fold related fields into a nested sub-object on the normalized shape. Nesting hides fields from grep and misleads a consumer who knows the wire into hunting for a value that should be top-level. If ≥2 of those fields are always consumed together by a function, model the grouping as a **function param type** the flat shape structurally satisfies (e.g. `SolverCloseFeeRates`), not as stored structure. (See root `AGENTS.md` Learned Rules.)
...
- **A version-grown output field stays on one shape, optional, `undefined` where the chain predates it.** When a newer contracts generation adds a field to a read's output (e.g. `WithdrawRequest.advancedAmount`, added in v0.8.6), keep the single result interface and mark the field optional; a `contractsVersion: "0.8.5"` chain decodes a pinned legacy fragment and returns it as-is, so the field is `undefined` — never stamp a default (`0n`), never split per-version union types. Deliberately the opposite of per-**solver** divergence (a `kind`-discriminated union): solver kind is an axis the caller selects and passes, while the contracts generation is a per-chain deployment fact resolved from config — the parameter callers pass (`chainId`) is not the axis, so no compile-time narrowing is possible and a union would only add a synthetic discriminant to wire-shaped data.
