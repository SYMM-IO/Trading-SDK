...
import { getOpenInterestBySymbolIdQueryKey, getOpenInterestBySymbolIdQueryOptions } from "./query-open-interest";

describe("getOpenInterestBySymbolIdQueryOptions", () => {
  it("is enabled for a positive symbolId", () => {
    const { config } = mockConfig();
    expect(getOpenInterestBySymbolIdQueryOptions(config, { symbolId: 132 }).enabled).toBe(true);
  });

  it("is disabled when symbolId is 0 (the no-market sentinel)", () => {
    const { config } = mockConfig();
    expect(getOpenInterestBySymbolIdQueryOptions(config, { symbolId: 0 }).enabled).toBe(false);
  });

  it("respects an explicit query.enabled override even with a positive symbolId", () => {
    const { config } = mockConfig();
    expect(getOpenInterestBySymbolIdQueryOptions(config, { symbolId: 132, query: { enabled: false } }).enabled).toBe(
      false,
    );
  });
  ...
  it("queryFn surfaces a SymmError for an unsupported chain", async () => {
    const { config } = mockConfig();
    const options = getOpenInterestBySymbolIdQueryOptions(config, { chainId: mainnet.id, symbolId: 132 });
