...
vi.mock("../types/generated/rasa-solver", async (importOriginal) => {
  ...
  return { ...actual, checkInWhiteListCheckInWhitelistAddressMultiAccountAddressGet: genFn };
});

import { checkSolverWhitelistQueryKey, checkSolverWhitelistQueryOptions } from "./query";
...
describe("checkSolverWhitelistQueryOptions", () => {
  it("separates cache entries by solverId and includes the required address", () => {
    const a = checkSolverWhitelistQueryKey({ chainId: BASE, solverId: "rasa", address: USER });
    const b = checkSolverWhitelistQueryKey({ chainId: BASE, solverId: "enigma", address: USER });
    expect(a).not.toEqual(b);
    expect(JSON.stringify(a)).toContain(USER);
  });

  it("queryFn forwards the address and the MultiAccount default, not just the key", async () => {
    genFn.mockResolvedValue({ data: true });
    const spy = vi.spyOn(config, "getSolver");

    const options = checkSolverWhitelistQueryOptions(config, { chainId: BASE, solverId: "rasa", address: USER });
