...
describe("deleteSubAccount", () => {
  it("writes deleteSubAccount to the AccountLayer", async () => {
    const { config, writeContract } = mockConfig();

    const hash = await deleteSubAccount(config, { subAccount: SUB_ACCOUNT });
  ...
  it("throws when the config has no wallet resolver", async () => {
    ...
    await expect(deleteSubAccount(config, { subAccount: SUB_ACCOUNT })).rejects.toThrow(SymmError);
  });
  ...
  describe("pre-flight simulation", () => {
    it("dry-runs the call before writing by default", async () => {
      ...
      await deleteSubAccount(config, { subAccount: SUB_ACCOUNT });
    ...
    it("skips the dry-run when `simulateBeforeWrite` is false on the call", async () => {
      ...
      await deleteSubAccount(config, { subAccount: SUB_ACCOUNT, simulateBeforeWrite: false });
    ...
    it("aborts the write when the dry-run would revert", async () => {
      ...
      await expect(deleteSubAccount(config, { subAccount: SUB_ACCOUNT })).rejects.toThrow("would revert");
