...
import { requestToCancelQuote } from "./request-to-cancel-quote";

const DEFAULT = getChainConfig(SymmioSupportedChainId.HYPER_EVM);
const SUB_ACCOUNT: Address = "0xaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
const QUOTE_ID = 42n;

describe("requestToCancelQuote", () => {
  it("wraps the core requestToCancelQuote call in AccountLayer `_call`", async () => {
    const { config, writeContract } = mockConfig();

    const hash = await requestToCancelQuote(config, { account: SUB_ACCOUNT, quoteId: QUOTE_ID });
  ...
  it("dry-runs the routed `_call` before writing by default", async () => {
    ...
    await requestToCancelQuote(config, { account: SUB_ACCOUNT, quoteId: QUOTE_ID });
  ...
  it("throws when the config has no wallet resolver", async () => {
    ...
    await expect(requestToCancelQuote(config, { account: SUB_ACCOUNT, quoteId: QUOTE_ID })).rejects.toThrow(SymmError);
  });
});
