...
describe("forceCancelQuote", () => {
  it("wraps the core forceCancelQuote call in AccountLayer `_call`", async () => {
    const { config, writeContract } = mockConfig();

    const hash = await forceCancelQuote(config, { account: SUB_ACCOUNT, quoteId: QUOTE_ID });
  ...
  it("throws when the config has no wallet resolver", async () => {
    ...
    await expect(forceCancelQuote(config, { account: SUB_ACCOUNT, quoteId: QUOTE_ID })).rejects.toThrow(SymmError);
  });
});
