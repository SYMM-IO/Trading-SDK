...
const ARBITRUM_SYMMIO = getChainConfig(SymmioSupportedChainId.ARBITRUM).addresses.symmioAddress;
...
describe("resolvePoolSource", () => {
  it("lower-cases the diamond so the subgraph's Bytes equality filter matches", () => {
    ...
    expect(ARBITRUM_SYMMIO).not.toBe(ARBITRUM_SYMMIO.toLowerCase());
    expect(resolvePoolSource(config, SymmioSupportedChainId.ARBITRUM)).toBe(ARBITRUM_SYMMIO.toLowerCase());
  });

  it("falls back to the config's default chain when no chainId is passed", () => {
    const { config } = mockConfig();

    expect(resolvePoolSource(config)).toBe(ARBITRUM_SYMMIO.toLowerCase());
  });
