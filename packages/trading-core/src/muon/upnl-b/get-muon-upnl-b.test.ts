...
import { getMuonUpnlB } from "./get-muon-upnl-b";
...
describe("getMuonUpnlB", () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });

  it("fetches and normalizes the uPnl_B attestation from the first oracle URL", async () => {
    const get = vi.spyOn(axios, "get").mockResolvedValue({ data: RAW });

    const result = await getMuonUpnlB(config, { partyB: PARTY_B, partyA: PARTY_A });
  ...
  it("throws a SymmError when every oracle URL fails", async () => {
    ...
    await expect(getMuonUpnlB(config, { partyB: PARTY_B, partyA: PARTY_A })).rejects.toBeInstanceOf(SymmError);
  });
  ...
  it("throws a SymmError when every oracle returns an unsuccessful response", async () => {
    ...
    await expect(getMuonUpnlB(config, { partyB: PARTY_B, partyA: PARTY_A })).rejects.toThrow(SymmError);
  });
});
