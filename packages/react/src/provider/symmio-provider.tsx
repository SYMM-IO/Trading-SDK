 ...
 */
export function SymmioProvider({ children, chains, defaultChainId }: SymmioProviderProps) {
  ...
  const config = useMemo<Config>(
    () =>
      createConfig({
        ...
        getClient: ({ chainId } = {}): PublicClient => {
          ...
          if (!client) {
            throw new SymmError(`No public client available for chain ${chainId ?? "(default)"}.`);
          }
          return client;
        },
    ...
    [wagmiConfig, chains, defaultChainId],
  );
