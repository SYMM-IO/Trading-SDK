...
function mockMutationFn(mutationFn: ReturnType<typeof vi.fn>) {
  instantCloseMutationOptions.mockReturnValue({ mutationKey: ["instantClose"], mutationFn });
}

describe("useInstantClose", () => {
  afterEach(() => {
    instantCloseMutationOptions.mockReset();
  });
  ...
  it("calls the core mutationFn with the connected chain and returns the result", async () => {
    ...
    const { result } = renderHookWithProviders(() => useInstantClose({ config }));
  ...
  it("invalidates the instant-closes feed (not the opens feed) for the active config on success", async () => {
    ...
    const { result } = renderHookWithProviders(() => useInstantClose({ config }), { queryClient });
