...
vi.mock("@theoldvarorg/core", async (importOriginal) => {
  ...
  return { ...actual, instantOpenAutoMutationOptions };
});

import { useInstantOpenAuto } from "./use-instant-open-auto";

const PARTY_A = "0x00000000000000000000000000000000000000a1" as const;
const VARS: PrepareInstantOpenParameters = {
...
function mockMutationFn(mutationFn: ReturnType<typeof vi.fn>) {
  instantOpenAutoMutationOptions.mockReturnValue({ mutationKey: ["instantOpenAuto"], mutationFn });
}

describe("useInstantOpenAuto", () => {
  afterEach(() => {
  ...
  it("calls the core mutationFn with the connected chain and returns the result", async () => {
    ...
    const { result } = renderHookWithProviders(() => useInstantOpenAuto({ config }));
