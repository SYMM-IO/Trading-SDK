...
import { useFinalizeWithdrawRequest } from "./use-finalize-withdraw-request";

const DEFAULT = getChainConfig(SymmioSupportedChainId.HYPER_EVM);
const SUB_ACCOUNT: Address = "0xaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
const REQUEST_ID = 1n;

describe("useFinalizeWithdrawRequest", () => {
  it("rejects with kind 'sdk' when no wallet is connected, without writing", async () => {
    ...
    const { result } = renderHookWithProviders(() => useFinalizeWithdrawRequest({ config }));

    let error: unknown;
    await act(async () => {
      try {
        await result.current.mutateAsync({ user: SUB_ACCOUNT, requestId: REQUEST_ID });
      } catch (err) {
        error = err;
      }
    });
  ...
  it("writes finalizeWithdrawRequest and returns the tx hash when waitForReceipt is false", async () => {
    const { config, writeContract } = createMockSymmioConfig();
    writeContract.mockResolvedValueOnce(TEST_TX_HASH);

    const { result } = renderHookWithProviders(() => useFinalizeWithdrawRequest({ config, waitForReceipt: false }));

    let res: unknown;
    await act(async () => {
      res = await result.current.mutateAsync({ user: SUB_ACCOUNT, requestId: REQUEST_ID });
    });

    expect(res).toEqual({ hash: TEST_TX_HASH });
    const call = writeContract.mock.calls[0]![0];
    expect(call.address).toBe(DEFAULT.addresses.symmioAddress);
    expect(call.functionName).toBe("finalizeWithdrawRequest");
    expect(call.args).toEqual([SUB_ACCOUNT, REQUEST_ID]);
  });
