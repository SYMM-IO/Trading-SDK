...
import { useAllocate } from "./use-allocate";
...
describe("useAllocate", () => {
  it("rejects with kind 'sdk' when no wallet is connected, without writing", async () => {
    ...
    const { result } = renderHookWithProviders(() => useAllocate({ config }));
  ...
  it("writes _call and returns the tx hash when waitForReceipt is false", async () => {
    ...
    const { result } = renderHookWithProviders(() => useAllocate({ config, waitForReceipt: false }));
