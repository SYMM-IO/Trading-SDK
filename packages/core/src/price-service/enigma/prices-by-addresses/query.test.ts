...
describe("getEnigmaPriceServicePricesByAddressesQueryOptions", () => {
  it("respects an explicit query.enabled override", () => {
    const { config } = mockConfig();
    expect(
      getEnigmaPriceServicePricesByAddressesQueryOptions(config, {
        addresses: ["0xabc"],
        query: { enabled: false },
      }).enabled,
    ).toBe(false);
  ...
  it("is enabled by default when parameters are provided", () => {
    const { config } = mockConfig();
    expect(getEnigmaPriceServicePricesByAddressesQueryOptions(config, { addresses: ["0xabc"] }).enabled).toBe(true);
  });
