...
`@symmio/ui` is the **design system** for this monorepo's own applications (`apps/web`, `apps/storybook`). It owns reusable UI primitives — buttons, inputs, modals, popovers, tooltips, tables, tabs, badges, layout primitives — and the design tokens that style them.

## Not Part of the SDK

This package is **not** part of the public SDK. Third-party consumers of `@symmio/trading-core` and `@symmio/trading-react` are not expected to depend on `@symmio/ui` — they bring their own UI. Do not put SDK logic, domain types, or business rules in here.
...
- **No coupling to `@symmio/trading-core` or `@symmio/trading-react`.** This package must remain installable on its own.
