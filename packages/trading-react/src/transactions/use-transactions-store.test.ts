...
describe("useTransactionsStore", () => {
  it("adds a tx in 'pending' status with a createdAt timestamp", () => {
    useTransactionsStore.getState().add({ hash: HASH_A, chainId: 42161, label: "Rename" });
  ...
  it("ignores duplicate hashes on add", () => {
    useTransactionsStore.getState().add({ hash: HASH_A, chainId: 42161, label: "Rename" });
    useTransactionsStore.getState().add({ hash: HASH_A, chainId: 42161, label: "Rename again" });
  ...
  it("clears every tx", () => {
    useTransactionsStore.getState().add({ hash: HASH_A, chainId: 42161, label: "A" });
    useTransactionsStore.getState().add({ hash: HASH_B, chainId: 42161, label: "B" });
    useTransactionsStore.getState().clear();
