...
describe("useRequestToCancelQuote", () => {
  it("rejects with kind 'sdk' when no wallet is connected, without writing", async () => {
    ...
    const { result } = renderHookWithProviders(() => useRequestToCancelQuote({ config }));
  ...
  it("writes requestToCancelQuote via _call and returns the tx hash when waitForReceipt is false", async () => {
    const { config, writeContract } = createMockSymmioConfig();
    writeContract.mockResolvedValueOnce(TEST_TX_HASH);

    const { result } = renderHookWithProviders(() => useRequestToCancelQuote({ config, waitForReceipt: false }));
