...
import { useForceCancelCloseRequest } from "./use-force-cancel-close-request";

const DEFAULT = getChainConfig(SymmioSupportedChainId.HYPER_EVM);
const SUB_ACCOUNT: Address = "0xaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
const QUOTE_ID = 42n;

describe("useForceCancelCloseRequest", () => {
  it("rejects with kind 'sdk' when no wallet is connected, without writing", async () => {
    ...
    const { result } = renderHookWithProviders(() => useForceCancelCloseRequest({ config }));
  ...
  it("writes forceCancelCloseRequest via _call and returns the tx hash when waitForReceipt is false", async () => {
    const { config, writeContract } = createMockSymmioConfig();
    writeContract.mockResolvedValueOnce(TEST_TX_HASH);

    const { result } = renderHookWithProviders(() => useForceCancelCloseRequest({ config, waitForReceipt: false }));
