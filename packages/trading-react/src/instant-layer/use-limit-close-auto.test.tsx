...
const limitCloseAutoMutationOptions = vi.hoisted(() => vi.fn());

vi.mock("@symmio/trading-core", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@symmio/trading-core")>();
  return { ...actual, limitCloseAutoMutationOptions };
});

import { useLimitCloseAuto } from "./use-limit-close-auto";

const PARTY_A = "0x00000000000000000000000000000000000000a1" as const;
const VARS: PrepareLimitCloseParameters = {
  partyA: PARTY_A,
  market: { id: 1 },
...
function mockMutationFn(mutationFn: ReturnType<typeof vi.fn>) {
  limitCloseAutoMutationOptions.mockReturnValue({ mutationKey: ["limitCloseAuto"], mutationFn });
}

describe("useLimitCloseAuto", () => {
  afterEach(() => {
    limitCloseAutoMutationOptions.mockReset();
  });
  ...
  it("calls the core mutationFn with the connected chain and returns the result", async () => {
    ...
    const { result } = renderHookWithProviders(() => useLimitCloseAuto({ config }));
  ...
  it("invalidates the instant-closes feed (not the opens feed) for the active config on success", async () => {
    ...
    const { result } = renderHookWithProviders(() => useLimitCloseAuto({ config }), { queryClient });
